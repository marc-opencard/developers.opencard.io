...
description: Reference for the api.opencard.io EMS API — manage TPAs, organizations, card holders, OAuth clients, and webhooks for Expense Management Systems.
---
...
| Payment Products | Catalog of cards → enable on your account |
...
1. `GET /payment-products` → `POST .../payment-products/{id}`
2. `POST /accounts/{id}/tpas` + signatories (`payment_product_id` = product `id`)
3. `POST /accounts/{id}/organizations`
4. `POST .../webhooks`
5. `POST .../cardholders`
6. Receive `card.transaction.*` on your webhook URL
