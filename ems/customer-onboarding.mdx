...
Start from the [EMS overview](/ems) if you want the big picture (webhooks + what to build in your product) before this flow.

Each major step has its own guide — same pattern as [TPA flow](/ems/tpa-flow) and [Card holders](/ems/card-holders):

| Guide | When |
|-------|------|
| [Payment product setup](/ems/payment-products-setup) | Once per EMS account — enable products before any client onboards |
...
sequenceDiagram
    ...
    You->>OC: GET /payment-products + enable on account
    Note over Client,OC: Phase 1 — Client setup
    Client->>You: Choose payment product (marketplace)
    You->>OC: GET account payment-products (enabled)
...
The client walks through your product: **payment product → add-ons → invoice details → TPA signatories**. You then create billing, TPA, organization, and webhook via the API (or receive the payload from the [ocTPA plugin](/ems/plugins) and POST it yourself).
...
→ [Payment product setup](/ems/payment-products-setup) · [TPA flow](/ems/tpa-flow) · [Billing](/ems/model/billing) · [Webhooks](/ems/webhooks/setup)
...
|---|------|--------------|--------------|
| 0 | Enable payment product(s) on account (once) | `GET /payment-products` → `POST .../accounts/{id}/payment-products/{id}` (or [application.opencard.io](https://application.opencard.io)) | [Payment product setup](/ems/payment-products-setup) |
| 1 | Client picks payment product | `GET .../accounts/{id}/payment-products` → `payment_product_id` on TPA | [Payment product setup](/ems/payment-products-setup) |
...
| **[ocTPA plugin](/ems/plugins)** | Ready-made wizard: payment product → add-ons → invoice → signatories; **you still POST** billing / TPA / signatories from `onDataSend` |
