...
<Tip>
  Card holders come **after** client setup (billing, TPA, organization, webhook). See the [full onboarding flow](/ems/customer-onboarding) if you have not finished Phase 1 yet.
</Tip>
...
| **Client already has issuer feed?** | → [Current card issuer integration](/ems/current-card-issuer-integration) | Same |
...
| | 🅱️ Match on employee number | 🅰️ Email + eID |
|--|------------------------------|----------------|
| **How** | Map the issuer’s employee number to yours → create with `identity_id` | Create with `email` + your `reference_id` |
| **User action** | None | Opens OpenCard email and identifies with **eID** |
| **You need** | An up-to-date employee register in **both** places (issuer + your EMS) | A working email address for the person |
| **Transactions** | Start as soon as you create the card holder | Start only **after** they complete eID |
...
<Note>
  Client already receives transactions from the issuer another way? See [Current card issuer integration](/ems/current-card-issuer-integration).
</Note>
