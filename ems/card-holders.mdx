...
| **Client already has issuer feed?** | See [parallel feeds](#when-the-client-already-has-another-transaction-feed) | Same |
...
## When the client already has another transaction feed
...
**Retroactive batch on onboard.** When `card_holder.identified` fires, OpenCard replays transaction states since the latest card invoice for that person's cards on the TPA (Path 🅰️ and Path 🅱️ above). The same cleared purchases may already exist in your system from the other integration.
