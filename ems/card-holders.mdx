...
## Existing card issuer integration

Your client may already receive transactions from the same card program through another integration. Running OpenCard in parallel is common during rollout — you do not need to wait for the old feed to be switched off.

**Who disables the old integration?** That is between your client and their card issuer. OpenCard does not require the client to turn off an existing connection, and you are not responsible for shutting it down. If they want a single source only, the client coordinates that with the issuer.

**Handling two feeds at once:**

- OpenCard delivers `card.transaction.authorized` in real time. Many legacy integrations only send `cleared`, so you may see authorised events from OpenCard that never existed on the old feed.
- When a card holder is onboarded (`card_holder.identified`), OpenCard replays historical transaction states since the latest card invoice — see Path 🅰️ and Path 🅱️ above. Expect overlap with transactions your client already has from the other integration.
- Use `card_issuer_reference` on **`card.transaction.cleared`** to deduplicate against your existing store. **Cleared matches cleared** — compare cleared references only, not authorised ones. The same purchase can carry different `card_issuer_reference` values on `authorized` vs `cleared`; that varies by card issuer and OpenCard cannot normalize it across issuers.
- Upsert on OpenCard transaction `id` as your primary key. Use `card_issuer_reference` as a secondary dedup key on cleared events when merging with another feed.

**Gradual migration.** A practical pattern: start processing OpenCard per card holder when `card_holder.identified` arrives, deduplicate cleared events with `card_issuer_reference`, and let the client decide when — or if — to retire the old integration with the issuer.
