...
| **User action** | Click email → sign PDPC with BankID/MitID | None — instant |
...
4. User reads PDPC → checks consent → identifies with **eID** (BankID 🇸🇪, MitID 🇩🇰, BankID 🇳🇴, TUPAS 🇫🇮)
...
## Switching from legacy issuer feed

When you receive `card_holder.identified` (either path) → **stop the old issuer feed** for that person and dedupe on `card_issuer_reference` for cleared transactions.

→ [Migrating from Direct Issuer](/ems/migrating-from-issuer)
