...
description: authorized vs cleared vs deleted vs invoiced. The logic you need to get right.
---
...
## Migrating from direct issuer feed

When switching from a legacy issuer integration to OpenCard:

1. Stop old feed when you get `card_holder.identified`
2. OpenCard delivers all transactions since last credit card invoice
3. Dedupe using `card_issuer_reference` on `cleared` events — if you already have it from the old feed, skip

See [Migrating from Issuer](/ems/migrating-from-issuer).
