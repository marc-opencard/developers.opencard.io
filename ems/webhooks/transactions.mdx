...
5. **`card_issuer_reference` differs per state** — don't use it as your primary key; when merging with another feed, compare cleared-to-cleared only → [Parallel transaction feeds](/ems/parallel-transaction-feeds)
...
Full guidance → [Parallel transaction feeds](/ems/parallel-transaction-feeds)
