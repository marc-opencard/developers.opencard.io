...
| `receiptable` | boolean | `true` if a digital receipt exists at an OpenCard Receipt provider partner |
| `card_issuer` | string | Card program identifier (`name_system`, e.g. `nordea`) |
| `payment_product` | object | `{ "id": <payment_product_id>, "name": "<product name>" }` |
