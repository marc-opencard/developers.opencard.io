...
description: PDPC GDPR consent flow for card holders — email invitation plus eID signing in Sweden, Norway, Denmark, and Finland before transactions start flowing.
---
...
`reference_id` is **required**. `email` is **optional** — omit to leave unchanged (including identity-linked holders with no email).

Resends PDPC email only when PDPC is unsigned or no identity is linked, `skip_pdpc_email=false`, **and** the card holder has an email address.
...
**Reference ID + new email (may trigger resend):**
