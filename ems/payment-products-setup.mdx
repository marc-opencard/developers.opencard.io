...
flowchart TD
    ...
    F --> G[TPA with card_issuer_id from the product id]
```
...
Build a marketplace (or wizard step) where the client picks one product. Store the chosen **`id`** — that becomes `card_issuer_id` when you create the TPA.
...
| `payment_program.scheme.logo_url` | Visa / Mastercard badge |
| `issuer.logo_url` | Bank mark when present |
| `id` | **`card_issuer_id`** on `POST .../tpas` |
| `card_issuer` | Stable product code — filter a subset with `allowedCardIssuerIds` in the plugin |
