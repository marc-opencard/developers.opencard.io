...
flowchart TD
    ...
    F --> G[TPA with payment_product_id]
```
...
Build a marketplace (or wizard step) where the client picks one product. Store the product **`id`** — you pass that as `payment_product_id` when you create the TPA.
...
| `id` | Pass this as `payment_product_id` on `POST .../tpas` |

---

## Next step

Once products are enabled, each client picks one in Phase 1 and you create the TPA with that `id` as `payment_product_id`.

→ [TPA flow](/ems/tpa-flow) · [Customer onboarding](/ems/customer-onboarding#step-by-step)
