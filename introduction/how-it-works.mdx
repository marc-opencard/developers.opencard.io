...
├── Billing      ← invoice recipient; orgs with same billing_id = one line on YOUR OpenCard invoice
└── Organization ← your end-client company
    ├── billing_id → which billing profile (invoice line roll-up) this org belongs to
...
**Billing** = invoice recipient for **your** OpenCard bill. Organizations that share a `billing_id` are rolled up as **one line** on that invoice (not separate invoices per org). Example: three orgs with `billing_id: 1` → one combined line to your finance team. → [Billing](/ems/model/billing)
...
flowchart TD
    ...
    B --> C[3. Enable payment products on account]
